...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:2fc2f9f5ed874803820365ede31e6892@localhost:5432/api_monitor',
...
const createDatabase = async () => {
  console.log('[PostgreSQL]  Using Railway PostgreSQL - skipping database creation')
}
  ...
  try {
    await pool.query(
      `INSERT INTO api_metrics (api_key, endpoint, method, status_code, response_time, timestamp)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [logData.apiKey, logData.endpoint, logData.method, logData.statusCode, logData.responseTime, logData.timestamp || new Date()]
    )
    await pool.query(
      `INSERT INTO api_summary (api_key, total_requests, total_errors, avg_response_time, last_updated)
       ...
       ON CONFLICT (api_key) DO UPDATE SET
         ...
         last_updated = NOW()`,
      [logData.apiKey, logData.statusCode >= 400 ? 1 : 0, logData.responseTime]
    )
